/*
 * jQuery plugin to show images and pdf files preview
 * https://github.com/guillermodiazga/EZView
 *
 * Copyright 2017, Guillermo Diaz
 *
 * Licensed under the MIT license:
 * https://opensource.org/licenses/MIT
 */

$.fn.extend({
    EZView: function(){
            var arIndex = [];
            var index = 0;
            
            var nextIcon = 'data:image/png;base64,';
            var backIcon = 'data:image/png;base64,';
            var downloadIcon = 'data:image/png;base64,';
            var zoominIcon = 'data:image/png;base64,';
            var zoomoutIcon = 'data:image/png;base64,';
            var closeIcon = 'data:image/png;base64,';
            
            var builtTemplate = function(index){
                var href = arIndex[index]['href'],
                    name = arIndex[index]['name'],
                    isPdf = href.match('.pdf'),
                    isImg = href.match('(\.jpg|\.png|\.jpeg|\.gif)$');
                
                var tools = '<div class="tools">'+
                        '<spam class="name"><b>'+name+'</b></spam>'+
                        '<a class="download EZViewHighlight" download href="'+href+'">'+
                        '<img src="'+downloadIcon+'" alt="Download" /></a>';
                
                if (isImg) {
                    
                tools +='<a class="zoomin EZViewHighlight" href="">'+
                        '<img src="'+zoominIcon+'" alt="Download" /></a>'+
                        '<a class="zoomout EZViewHighlight" href="">'+
                        '<img src="'+zoomoutIcon+'" alt="Download" /></a>';
                } 
                        
                tools +='<a class="close EZViewHighlight" href="">'+
                        '<img src="'+closeIcon+'"/></a>'+
                    '</div>';
                    
                // Content to show 
                var content = tools+'<h1 style="padding: 10px; border-radius: 10px; background-color: rgba(255,255,255,0.6)">Unsupported preview</h1>'+tools;
                
                // To show pdf files
                if (isPdf) {
                    content = tools+'<embed height="'+$(window).height()*0.95+'" width="'+$(window).width()*0.9+
                        '" src="'+href+'" type="application/pdf" />';
                }
                
                // To show images
                if (isImg) {
                    content = tools+'<img src="'+href+'" class="content" />';
                }

                var container = 
                    '<table align="center" style="height:100%; width:100%"><tr>'+
                        '<td width="10%" align="center" valign="middle">';
                    
                    if (index > 0) {
                        container+='<a class="back EZViewHighlight"><img src="'+backIcon+'" alt="Back" /></a>';
                    }
                    
                    container+='</td>'+
                        '<td width="80%" align="center" valign="middle">'+content+'</td>'+
                        '<td width="10%" align="center" valign="middle">';
                    
                    if (index < arIndex.length-1) {
                        container +=
                            '<a class="next EZViewHighlight"><img src="'+nextIcon+'" alt="Next" /></a>';
                    }
                        
                    container += '</td>'+
                    '</tr></table>';
                
                var template = '<div id="EZView" class="hide container">'+container+'</div>';
                
                return template;
            };
            
            var appendTemplate = function(template){
                
                // Append template to body
                 $('body').append(template);

                // Set Styles
                $('#EZView').css({
                    'background-color': 'rgba(0,0,0,0.5)',
                    'height':           '100%',
                    'width':            '100%',
                    'z-index':          '10000',
                    'position':         'fixed',
                    'top':              '0',
                    'left':             '0',
                    'cursor':           'default',
                    'aling-items':      'center',
                    'margin':           'auto',
                })
                .find('table td').css({
                    'vertical-align': 'middle',
                }).end()
                .find('.content').css({
                    'max-width': $(window).width()*0.7,
                    'max-height': $(window).height()*0.9,
                    'z-index':          '10001',
                }).end()
                .find('.name').css({
                    'left': '0',
                    'position': 'fixed',
                    'padding': '7px',
                }).end()
                .find('.close>img').css({
                    'top': '0',
                    'right': '0',
                    'position': 'fixed',
                }).end()
                .find('.back>img').css({
                    'top': $(window).height()*0.5,
                    'left': '0',
                    'position': 'fixed',
                }).end()
                .find('.next>img').css({
                    'top': $(window).height()*0.5,
                    'right': '0',
                    'position': 'fixed',
                }).end()
                .find('.tools').css({
                    'background-color': 'rgba(255,255,255,0.5)',
                    'height':           '35px',
                    'width':            '100%',
                    'position':         'fixed',
                    'top':              '0',
                    'left':             '0',
                    'padding':          '3px',
                    'padding-top':       '0px',
                    'z-index':          '100000'
                }).end()
                .find('.tools>a').css({
                    'margin' : '10px',
                }).end()
                .find('.EZViewHighlight>img').css({
                    'padding':          '10px',
                    'height':           'auto',
                    'width':            'auto',
                    'z-index':          '100000'
                }).end()
                .find('.EZViewHighlight>img').hover(
                    function(){
                        $(this).css({
                            'border-radius':    '100%',
                            'background-color': 'rgba(255,255,255,0.5)',
                            'z-index': '100000'
                        })
                    },
                    function(){
                        $(this).css({
                            'background-color': '',
                            'border-radius':    '',
                        })
                    },
                );
            };
            
            var next = function(){
                var newIndex = index+1;

               if (arIndex[newIndex]) {
                    index = newIndex;
                    
                    $('#EZView').remove();
                    appendTemplate(builtTemplate(index));
                    addEvents();
                    
                    // Show EZView
                    $( "#EZView" ).show();
                }
            }
            
            var back = function(){
                var newIndex = index-1;

                if (arIndex[newIndex]) {
                    index = newIndex;
                    
                    $('#EZView').remove();
                    appendTemplate(builtTemplate(index));
                    addEvents();

                    // Show EZView
                    $( "#EZView" ).show();
                }
            }
            
            
            var keyupEvents = function(e){
                var keyCode = e.keyCode;

                switch(keyCode){
                // Esc
                case 27:
                    close();
                    break;
                    
                // Next 
                case 39:
                    next();
                    break;
                
                // Back
                case 37:
                    back();
                    break;
                
                // zoomin
                case 107:
                    zoom(true);
                    break;

                // zoomout
                case 109:
                    zoom();
                    break;
                }
            }
            
            var close = function (e){
                // Remove EZView elements
                $('#EZView').remove();

                // Remove keyup events
                $(window).off('keyup', null, keyupEvents);
            }
            
            var zoom = function (increment){
                 var $img   = $('.content', '#EZView'),
                     height = parseInt($img.css('height')),
                     width  = parseInt($img.css('width'));
                     
                // Avoid lose imng proportions on zoom in
                $('#EZView').find('.content').css({
                    'max-width': '',
                    'max-height': '',
                })
                
                // Enable drag
                .draggable()
                .css({'cursor': 'move'});

                // Increase or decrease size
                if (increment) {
                    height+=height*0.2;
                    width+=width*0.2;
                }else{
                    height-=height*0.2;
                    width-=width*0.2;
                }
                
                $img.animate({
                    'height': (height)+'px',
                    'width': (width)+'px'
                }, 200);
            };
            
            var addEvents = function(){
                
                // Add keyup events
                $(window).off('keyup', null, keyupEvents).keyup(keyupEvents);

                // Main container
                $('#EZView')
                
                // Add close Event
                .click(function(e){
                    e.stopPropagation();
                    e.preventDefault();
                    
                    close();
                })
                 
                
                // Add close Event
                .find('.close>img').click(function(e){
                    e.preventDefault();
                    e.stopPropagation();
                    close();
                }).end()
                
                // Stop propagation
                .find('.download, img, .tools').click(function(e){
                    // Avoid trigger remove action
                    e.stopPropagation();
                }).end()
                
                // Add back event
                .find('.back>img').click(function(e){
                    e.stopPropagation();
                    e.preventDefault();
                    back();
                }).end()
                
                // Add next event
                .find('.next>img').click(function(e){
                    e.stopPropagation();
                    e.preventDefault();
                    next();
                }).end()
                
                // Add prevent default
                .find('.zoomin, .zoomout, .close').click(function(e){
                    e.stopPropagation();
                    e.preventDefault();
                }).end()

                // Add zoomin event
                .find('.zoomin>img').click(function(e){
                    e.stopPropagation();
                    e.preventDefault();
                    zoom(true);
                }).end()
                
                // Add zoomout event
                .find('.zoomout>img').click(function(e){
                    e.stopPropagation();
                    e.preventDefault();
                    zoom();
                }).end();
            };

            // Constructor
            return $(this).each(function(i, el){
                    arIndex[i] = [];
                    arIndex[i]['href'] = $(el).attr('href') || $(el).attr('src');
                    arIndex[i]['name'] = $.trim($(el).html());
                    
                    // Set index on DOM element
                    $(el).attr('index', i);

                    // Add events to each elements
                    $(this).click(function(e){
                        e.preventDefault();
                        e.stopPropagation();

                        index = parseInt($(e.target).attr('index'));
                        
                        if ($('#EZView').size()) {
                        // Remove exist template
                            $('#EZView').remove();
                        }
                        
                        // Get href attr
                        var href = arIndex[i]['href'],

                        template = builtTemplate(index);

                        appendTemplate(template);

                        addEvents();
                        
                        // Show EZView
                        $( "#EZView" ).show();

                    });
            });
        }
    }    
);
